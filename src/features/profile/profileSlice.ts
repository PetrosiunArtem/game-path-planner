...
const profileSlice = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addMatcher(
                (action) => [toggleWeapon.fulfilled.type, updateSkillLevel.fulfilled.type, toggleBoss.fulfilled.type, cycleLevelStatus.fulfilled.type].includes(action.type),
...
export const toggleBoss = createAsyncThunk(
    ...
    async (id: string, { getState }) => {
        const state = getState() as { profile: ProfileState };
        const profile = state.profile;
        const newBosses = profile.bosses.map(b => b.id === id ? { ...b, defeated: !b.defeated } : b);
